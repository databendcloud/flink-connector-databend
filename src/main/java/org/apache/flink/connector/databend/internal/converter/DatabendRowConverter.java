...
public class DatabendRowConverter implements Serializable {
    ...
    protected DatabendRowConverter.DeserializationConverter createToInternalConverter(LogicalType type) {
        switch (type.getTypeRoot()) {
            ...
            case ARRAY:
            case MAP:
                return val -> DatabendConverterUtils.toInternal(val, type);
            case ROW:
            case MULTISET:
            case RAW:
            default:
        ...
        switch (type.getTypeRoot()) {
            ...
            case TIMESTAMP_WITHOUT_TIME_ZONE:
                ...
                return (val, index, statement) -> statement.setTimestamp(
                        index + 1, val.getTimestamp(index, timestampPrecision).toTimestamp());
            case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
                final int localZonedTimestampPrecision = ((LocalZonedTimestampType) type).getPrecision();
                return (val, index, statement) -> statement.setTimestamp(
                        index + 1,
                        Timestamp.from(val.getTimestamp(index, localZonedTimestampPrecision)
                                .toInstant()));
            case DECIMAL:
            ...
            case ARRAY:
                return (val, index, statement) -> statement.setArray(
                        index + 1, (Array) DatabendConverterUtils.toExternal(val.getArray(index), type));
            case MAP:
                return (val, index, statement) ->
                        statement.setObject(index + 1, DatabendConverterUtils.toExternal(val.getMap(index), type));
            case MULTISET:
            case ROW:
            case RAW:
            default:
    ...
    interface DeserializationConverter extends Serializable {
        /**
         * Convert a object of {@link DatabendResultSet} to the internal data structure object.
         */
