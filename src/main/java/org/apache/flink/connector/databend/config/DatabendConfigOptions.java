...
public class DatabendConfigOptions {
    public static final ConfigOption<String> URL = ConfigOptions.key(DatabendConfig.URL)
            .stringType()
            .noDefaultValue()
            .withDescription("The Databend url in format `databend://<host>:<port>`.");

    public static final ConfigOption<String> USERNAME = ConfigOptions.key(DatabendConfig.USERNAME)
            .stringType()
            .noDefaultValue()
            .withDescription("The Databend username.");

    public static final ConfigOption<String> PASSWORD = ConfigOptions.key(DatabendConfig.PASSWORD)
            .stringType()
            .noDefaultValue()
            .withDescription("The Databend password.");

    public static final ConfigOption<String> DATABASE_NAME = ConfigOptions.key(DatabendConfig.DATABASE_NAME)
            .stringType()
            .defaultValue("default")
            .withDescription("The Databend database name. Default to `default`.");

    public static final ConfigOption<String> DEFAULT_DATABASE = ConfigOptions.key(
                    CommonCatalogOptions.DEFAULT_DATABASE_KEY)
            .stringType()
            .noDefaultValue()
            .withDescription("The Databend default database name.");

    public static final ConfigOption<String> TABLE_NAME = ConfigOptions.key(DatabendConfig.TABLE_NAME)
            .stringType()
            .noDefaultValue()
            .withDescription("The Databend table name.");

    public static final ConfigOption<Integer> SINK_BATCH_SIZE = ConfigOptions.key(DatabendConfig.SINK_BATCH_SIZE)
            .intType()
            .defaultValue(1000)
            .withDescription(
                    "The max flush size, over this number of records, will flush data. The default value is 1000.");

    public static final ConfigOption<Duration> SINK_FLUSH_INTERVAL = ConfigOptions.key(
                    DatabendConfig.SINK_FLUSH_INTERVAL)
            .durationType()
    ...
    public static final ConfigOption<Integer> SINK_MAX_RETRIES = ConfigOptions.key(DatabendConfig.SINK_MAX_RETRIES)
            .intType()
            .defaultValue(3)
            .withDescription("The max retry times if writing records to database failed.");

    public static final ConfigOption<SinkUpdateStrategy> SINK_UPDATE_STRATEGY = ConfigOptions.key(
                    DatabendConfig.SINK_UPDATE_STRATEGY)
